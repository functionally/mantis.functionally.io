...
| jq '{ date : "'$TIMESTAMP'" , "SOFR_1MONTH" : [ .quoteResponse.result | .[] | select(.underlyingSymbol == "SR1.CME") | { (.symbol) : { settle : .expireIsoDate[0:10], open : (.regularMarketOpen | tostring) } } ] | add  , "SOFR_3MONTH" : [ .quoteResponse.result | .[] | select(.underlyingSymbol == "SR3.CME") | { (.symbol) : { settle : .expireIsoDate[0:10], open : (.regularMarketOpen | tostring) } } ] | add  }' \
